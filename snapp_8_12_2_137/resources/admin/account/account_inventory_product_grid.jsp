...
<v:grid search="<%=ansDO%>" id="inventory-balance-grid">
  ...
  <tbody>
    <v:grid-row search="<%=ansDO%>">
      ...
      <td>
        <a href="<v:config key="site_url"/>/admin?page=product&id=<%=record.ProductId.getEmptyString()%>"><%=record.ProductName.getHtmlString()%></a><br/>
...
function showTransactionHistory(accountInventoryBalanceId) {
	asyncDialogEasy('account/account_inventory_transaction_dialog', 'AccountInventoryBalanceId=' + accountInventoryBalanceId);
}
