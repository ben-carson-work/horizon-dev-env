...
<% 
  String[] transactionIDs = pageBase.getDB().executeQuery(
    "select TransactionId from tbTransactionItem where SaleItemId in" + JvString.CRLF +
    "  (select SaleItemId from tbSaleItemVoucher where VoucherId=" + JvString.sqlStr(pageBase.getId()) + ")" + JvString.CRLF +
    "union" + JvString.CRLF +
    "select SrcEntityId from tbEntityLink where DstEntityType=29 and DstEntityId=" + JvString.sqlStr(pageBase.getId()) + JvString.CRLF +
    "union" + JvString.CRLF +
    "select TransactionId from tbVoucher where VoucherId=" + JvString.sqlStr(pageBase.getId()) + JvString.CRLF + 
    "union" + JvString.CRLF +
    "select" + JvString.CRLF + 
    "  T.transactionId" + JvString.CRLF +
    "from" + JvString.CRLF + 
    "  tbPaymentCredit PC inner join" + JvString.CRLF +
    "  tbPayment P on P.PaymentId=PC.PaymentId inner join" + JvString.CRLF +
    "  tbTransaction T on T.TransactionId=P.TransactionId" + JvString.CRLF +
    "where" + JvString.CRLF + 
    "  VoucherId=" + JvString.sqlStr(pageBase.getId())).getStrings();
