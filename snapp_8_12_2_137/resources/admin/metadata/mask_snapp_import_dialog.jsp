...
<v:dialog id="mask-snapp-import-dialog" title="@Common.ImportFormHint" width="800" height="490" autofocus="false">
  <div id="step-upload">
    ...
    <v:alert-box type="info" title="@Common.Info" style="max-height:350px;overflow:auto">
      This wizard will import forms from a SnApp file into the system, the following operations for linked entities are supported:<br/><br/>
      Create
      <ul>
  ...
  <div id="step-preview" class="v-hidden">
    ...
    <v:widget caption="@Common.Parameters">
      <v:widget-block>
        <v:db-checkbox field="UpdateExistingMasks" caption="Update existing forms" value="true"/><br/><br/>
...
$(document).ready(function() {
  var $dlg = $("#mask-snapp-import-dialog");
  $dlg.on("snapp-dialog", function(event, params) {
	...
	$("#mask-snapp-import-dialog").find("[name='UpdateExistingMasks']").prop('checked', true);
	$("#mask-snapp-import-dialog").find("[name='UpdateLinkedEntities']").prop('checked', false);
	$("#mask-snapp-import-dialog").find("[name='UpdateLinkedEntities']").change(refreshWarning);
	
});

function refreshWarning() {
  var $warnDiv = $("#mask-snapp-import-dialog").find(".update-linked-entities-warning");
	$warnDiv.setClass("hidden", !$("#mask-snapp-import-dialog").find("[name='UpdateLinkedEntities']").isChecked());
}

function getImportParams() {
  return {
    Mask: {
    	UpdateExistingMasks: $("#mask-snapp-import-dialog [name='UpdateExistingMasks']").isChecked(),
    	UpdateLinkedEntities: $("#mask-snapp-import-dialog [name='UpdateLinkedEntities']").isChecked()
    }
  }
}

function doUploadFinish(obj) {
  resizeDialogWidth("#mask-snapp-import-dialog", 800);
  ...
  vgsService("AsyncProcess", reqDO, false, function(ansDO) {
	  var validImport = ansDO.Answer.SnAppImportPreview.ImportTypeCode == <%=LkSNExportType.Mask.getCode()%>;     
    ...
    if (validImport) {
      ...
      $("#button-ok").removeClass("v-hidden");
    	...
    	$('text[name=ImportTypeExpected]').append(<%=JvString.jsString(LkSNExportType.Mask.getHtmlDescription(pageBase.getLang()))%> + "<br/>"); 
...
function doImportMasks() {
  var reqDO = {
      Command: "SnAppImport",
      SnAppImport: {
        RepositoryId: repositoryId,
        AsyncProcessClassAlias: <%=JvString.jsString(AsyncProcessUtils.CLASS_ALIAS_IMPORT_SNP_MASK)%>, 
  ...
  vgsService("AsyncProcess", reqDO, false, function(ansDO) {
    showAsyncProcessDialog(ansDO.Answer.SnAppImport.AsyncProcessId, function() {
      triggerEntityChange(<%=LkSNEntityType.Mask.getCode()%>);    
    });
    $("#mask-snapp-import-dialog").dialog("close");
  });
}
