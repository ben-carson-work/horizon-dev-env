<%@page import="com.vgs.cl.*"%>
<%@page import="com.vgs.snapp.dataobject.*"%>
<%@page import="com.vgs.snapp.lookup.*"%>
<%@page import="com.vgs.cl.lookup.*"%>
<%@page import="com.vgs.web.library.*"%>
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ taglib uri="vgs-tags" prefix="v"%>
<%@ taglib uri="snp-tags" prefix="snp"%>

<jsp:useBean id="pageBase" class="com.vgs.web.page.PageLogAndApiTracing" scope="request"/>
<jsp:useBean id="rights" class="com.vgs.snapp.dataobject.DORightRoot" scope="request"/>

<%
String defaultDateXML = pageBase.getBrowserFiscalDate().getXMLDate();
pageBase.setDefaultParameter("FilterDate", defaultDateXML);
%>

<v:tab-toolbar>
  <v:button id="btn-search" caption="@Common.Search" fa="search" enabled="<%=rights.MonitorIT.getBoolean()%>"/>
  <v:button id="btn-config" caption="@Common.Settings" fa="sliders" enabled="<%=rights.SettingsITSettings.getBoolean()%>"/>
  
  <v:pagebox gridId="api-grid"/>
</v:tab-toolbar>

<v:tab-content include="<%=rights.MonitorIT.getBoolean()%>">
  <v:profile-recap>
    <v:widget caption="@Common.Filters">
      <v:widget-block>
        <v:itl key="@Common.Date"/><br/>
        <v:input-text type="datepicker" field="FilterDate"/> 
                      
        <div class="filter-divider"></div>
        
        <v:itl key="@Common.FromTime"/><br/>
        <v:input-text type="timepicker" field="TimeFrom" />
                      
        <div class="filter-divider"></div>

        <v:itl key="@Common.ToTime"/><br/>
        <v:input-text type="timepicker" field="TimeTo" />
      </v:widget-block>
      <v:widget-block>
        <div><v:db-checkbox field="AnswerCommonStatus" value="<%=LkCommonStatus.Active.getCode()%>" caption="Successful" legendColor="var(--base-green-color)"/></div>
        <div><v:db-checkbox field="AnswerCommonStatus" value="<%=LkCommonStatus.Deleted.getCode()%>" caption="Error" legendColor="var(--base-red-color)"/></div>
        <div><v:db-checkbox field="AnswerCommonStatus" value="<%=LkCommonStatus.FatalError.getCode()%>" caption="Fatal" legendColor="var(--base-purple-color)"/></div>
      </v:widget-block>
    </v:widget>

    <v:widget caption="@Common.Workstation">
      <v:widget-block>
        <v:itl key="@Account.Location"/><br/>
        <snp:dyncombo auditLocationFilter="true" id="LocationId" entityType="<%=LkSNEntityType.Location%>"/>
        
        <div class="filter-divider"></div>
        
        <v:itl key="@Account.OpArea"/><br/>
        <snp:dyncombo auditLocationFilter="true" id="OpAreaId" entityType="<%=LkSNEntityType.OperatingArea%>" parentComboId="LocationId"/>
        
        <div class="filter-divider"></div>
        
        <v:itl key="@Common.Workstation"/><br/>
        <snp:dyncombo auditLocationFilter="true" id="WorkstationId" entityType="<%=LkSNEntityType.Workstation%>" parentComboId="OpAreaId"/>
      </v:widget-block>
    </v:widget>

    <v:widget caption="Request">
      <v:widget-block>
        Request code<br/>
        <snp:dyncombo id="RequestCode" entityType="<%=LkSNEntityType.ApiRequestCode%>"/>
        
        <div class="filter-divider"></div>
        
        Request command<br/>
        <snp:dyncombo id="RequestCommand" entityType="<%=LkSNEntityType.ApiRequestCommand%>" parentComboId="RequestCode"/>
      </v:widget-block>
    </v:widget>
  </v:profile-recap>

  <v:profile-main>
    <v:async-grid id="api-grid" jsp="log/api_grid.jsp" autoload="false"/>
  </v:profile-main>
</v:tab-content>

<script>

$(document).ready(function() {
  $("#btn-search").click(_search);
  $("#btn-config").click(_config);
  _search();

  function _search() {
    setGridUrlParam("#api-grid", "StartDateTime", _getDateTimeFilter("#TimeFrom", false));
    setGridUrlParam("#api-grid", "EndDateTime", _getDateTimeFilter("#TimeTo", true));

    setGridUrlParam("#api-grid", "LocationId", ($("#LocationId").getComboIndex() <= 0) ? "" : $("#LocationId").val());
    setGridUrlParam("#api-grid", "OpAreaId", ($("#OpAreaId").getComboIndex() <= 0) ? "" : $("#OpAreaId").val());
    setGridUrlParam("#api-grid", "WorkstationId", ($("#WorkstationId").getComboIndex() <= 0) ? "" : $("#WorkstationId").val());
    setGridUrlParam("#api-grid", "RequestCode", ($("#RequestCode").getComboIndex() <= 0) ? "" : $("#RequestCode").val());
    setGridUrlParam("#api-grid", "RequestCommand", ($("#RequestCommand").getComboIndex() <= 0) ? "" : $("#RequestCommand").val());
    setGridUrlParam("#api-grid", "AnswerCommonStatus", $("[name='AnswerCommonStatus']").getCheckedValues());

    changeGridPage("#api-grid", "first");
  }
  
  function _getDateTimeFilter(field, max) {
    var date = $("#FilterDate-picker").getXMLDate();
    if (getNull(date) == null)
      date = <%=JvString.jsString(defaultDateXML)%>;

    var time = $(field).getXMLTime();
    if (getNull(time) == null)
      time = (max === true) ? "23:59" : "00:00";
    
    return date + "T" + time;
  }
  
  function _config() {
    asyncDialogEasy("log/apicfg_dialog");
  }
});

</script>