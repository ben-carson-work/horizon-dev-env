...
<v:dialog id="calendar-snapp-import-dialog" title="@Common.ImportCalendarHint" width="800" height="490" autofocus="false">
  <div id="step-upload">
    ...
    <v:alert-box type="info" title="@Common.Info" style="max-height:350px;overflow:auto">
      This wizard will import calendars from a SnApp file into the system, the following operations for linked entities are supported:<br/><br/>
      Create
      <ul>
        <li><b>Rate codes</b></li>
        <li><b>Performance types</b></li>
      </ul>
    </v:alert-box>
  </div>
  ...
  <div id="step-preview" class="v-hidden">
    ...
    <v:widget caption="@Common.Parameters">
      <v:widget-block>
        <v:db-checkbox field="UpdateExistingCalendars" caption="Update existing calendars" value="true"/><br/><br/>
...
$(document).ready(function() {
  var $dlg = $("#calendar-snapp-import-dialog");
  ...
  $("#calendar-snapp-import-dialog").find("[name='UpdateExistingCalendars']").prop('checked', true);
  $("#calendar-snapp-import-dialog").find("[name='UpdateLinkedEntities']").prop('checked', false);
  $("#calendar-snapp-import-dialog").find("[name='UpdateLinkedEntities']").change(refreshWarning);
  
});

function refreshWarning() {
  var $warnDiv = $("#calendar-snapp-import-dialog").find(".update-linked-entities-warning");
  $warnDiv.setClass("hidden", !$("#calendar-snapp-import-dialog").find("[name='UpdateLinkedEntities']").isChecked());
}

function getImportParams() {
  return {
    Calendar: {
    	UpdateExistingCalendars: $("#calendar-snapp-import-dialog [name='UpdateExistingCalendars']").isChecked(),
      UpdateLinkedEntities: $("#calendar-snapp-import-dialog [name='UpdateLinkedEntities']").isChecked()
    }
  }
}

function doUploadFinish(obj) {
  resizeDialogWidth("#calendar-snapp-import-dialog", 800);
  ...
  vgsService("AsyncProcess", reqDO, false, function(ansDO) {
    var validImport = ansDO.Answer.SnAppImportPreview.ImportTypeCode == <%=LkSNExportType.Calendar.getCode()%>;     
    ...
    if (validImport) {
      ...
      $('text[name=ImportTypeExpected]').append(<%=JvString.jsString(LkSNExportType.Calendar.getHtmlDescription(pageBase.getLang()))%> + "<br/>"); 
...
function doImportCalendars() {
  var reqDO = {
      Command: "SnAppImport",
      SnAppImport: {
        RepositoryId: repositoryId,
        AsyncProcessClassAlias: <%=JvString.jsString(AsyncProcessUtils.CLASS_ALIAS_IMPORT_SNP_CALENDAR)%>, 
  ...
  vgsService("AsyncProcess", reqDO, false, function(ansDO) {
    ...
    $("#calendar-snapp-import-dialog").dialog("close");
  });
}
