...
<v:dialog id="taxprofile-snapp-import-dialog" title="Import tax profiles" width="800" height="490" autofocus="false">
  <div id="step-upload">
    ...
    <v:alert-box type="info" title="@Common.Info" style="max-height:350px;overflow:auto">
      This wizard will import tax profiles from a SnApp file into the system.<br/><br/>
    </v:alert-box>
  </div>
  ...
  <div id="step-preview" class="v-hidden">
    ...
    <v:widget caption="@Common.Parameters">
      <v:widget-block>
        <v:db-checkbox field="UpdateExistingTaxProfile" caption="Update existing tax profiles" value="true"/><br/><br/>
      </v:widget-block> 
    </v:widget>
  </div>
...
$(document).ready(function() {
  var $dlg = $("#taxprofile-snapp-import-dialog");
  $dlg.on("snapp-dialog", function(event, params) {
	...
	$("#taxprofile-snapp-import-dialog").find("[name='UpdateExistingTaxProfile']").prop('checked', true);
});

function getImportParams() {
  return {
    TaxProfile: {
    	UpdateExistingTaxProfile: $("#taxprofile-snapp-import-dialog [name='UpdateExistingTaxProfile']").isChecked()
    }
  }
}

function doUploadFinish(obj) {
  resizeDialogWidth("#taxprofile-snapp-import-dialog", 800);
  ...
  vgsService("AsyncProcess", reqDO, false, function(ansDO) {
	  var validImport = ansDO.Answer.SnAppImportPreview.ImportTypeCode == <%=LkSNExportType.TaxProfile.getCode()%>;     
    ...
    if (validImport) {
      ...
      $('text[name=ImportTypeExpected]').append(<%=JvString.jsString(LkSNExportType.TaxProfile.getHtmlDescription(pageBase.getLang()))%> + "<br/>"); 
...
function doImportTaxProfiles() {
  var reqDO = {
      Command: "SnAppImport",
      SnAppImport: {
        RepositoryId: repositoryId,
        AsyncProcessClassAlias: <%=JvString.jsString(AsyncProcessUtils.CLASS_ALIAS_IMPORT_SNP_TAXPROFILE)%>, 
  ...
  vgsService("AsyncProcess", reqDO, false, function(ansDO) {
    ...
    $("#taxprofile-snapp-import-dialog").dialog("close");
  });
}
