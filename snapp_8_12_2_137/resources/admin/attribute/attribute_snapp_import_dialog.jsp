...
<v:dialog id="attribute-snapp-import-dialog" title="Import attributes" width="800" height="490" autofocus="false">
  <div id="step-upload">
    ...
    <v:alert-box type="info" title="@Common.Info" style="max-height:350px;overflow:auto">
      This wizard will import attributes from a SnApp file into the system, the following operations for linked entities are supported:<br/><br/>
      Create
      <ul>
        <li><b>Attributes</b></li>
        <li><b>Attribute items</b></li>
      </ul>
      Update
      <ul>
        <li><b>Attributes</b></li>
  ...
  <div id="step-preview" class="v-hidden">
    ...
    <v:widget caption="@Common.Parameters">
      <v:widget-block>
        <v:db-checkbox field="UpdateExistingAttributes" caption="Update existing attributes" value="true"/><br/><br/>
    ...
    <v:alert-box type="warning" clazz="update-linked-entities-warning hidden" title="IMPORTANT">
      <i><b>IMPORTANT: Updating linked entities can result in modifying other attributes behavior.</b></i>
    </v:alert-box>
  </div>
...
$(document).ready(function() {
  var $dlg = $("#attribute-snapp-import-dialog");
  ...
  $("#attribute-snapp-import-dialog").find("[name='UpdateExistingAttributes']").prop('checked', true);
  $("#attribute-snapp-import-dialog").find("[name='UpdateLinkedEntities']").prop('checked', false);
  $("#attribute-snapp-import-dialog").find("[name='UpdateLinkedEntities']").change(refreshWarning);
});

function refreshWarning() {
  var $warnDiv = $("#attribute-snapp-import-dialog").find(".update-linked-entities-warning");
  $warnDiv.setClass("hidden", !$("#attribute-snapp-import-dialog").find("[name='UpdateLinkedEntities']").isChecked());
}

function getImportParams() {
  return {
	  Attribute: {
    	UpdateExistingAttributes: $("#attribute-snapp-import-dialog [name='UpdateExistingAttributes']").isChecked(),
      UpdateLinkedEntities: $("#attribute-snapp-import-dialog [name='UpdateLinkedEntities']").isChecked()
    }
  }
}

function doUploadFinish(obj) {
  resizeDialogWidth("#attribute-snapp-import-dialog", 800);
  ...
  vgsService("AsyncProcess", reqDO, false, function(ansDO) {
    var validImport = ansDO.Answer.SnAppImportPreview.ImportTypeCode == <%=LkSNExportType.Attribute.getCode()%>;     
    ...
    if (validImport) {
      ...
      $('text[name=ImportTypeExpected]').append(<%=JvString.jsString(LkSNExportType.Attribute.getHtmlDescription(pageBase.getLang()))%> + "<br/>"); 
...
function doImportAttributes() {
  var reqDO = {
      ...
      SnAppImport: {
        RepositoryId: repositoryId,
        AsyncProcessClassAlias: <%=JvString.jsString(AsyncProcessUtils.CLASS_ALIAS_IMPORT_SNP_ATTRIBUTE)%>, 
  ...
  vgsService("AsyncProcess", reqDO, false, function(ansDO) {
    ...
    $("#attribute-snapp-import-dialog").dialog("close");
  });
}
