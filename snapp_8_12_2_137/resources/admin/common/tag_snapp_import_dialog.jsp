...
<v:dialog id="tag-snapp-import-dialog" title="@Common.ImportTagHint" width="800" height="490" autofocus="false">
  <div id="step-upload">
    ...
    <v:alert-box type="info" title="@Common.Info" style="max-height:350px;overflow:auto">
      This wizard will import tags & label groups from a SnApp file into the system.<br/>
    </v:alert-box>
  </div>
  ...
  <div id="step-preview" class="v-hidden">
    ...
    <v:widget caption="@Common.Parameters">
      <v:widget-block>
        <v:db-checkbox field="UpdateExistingTags" caption="Update existing tags/label groups" value="true"/><br/><br/>
        <v:db-checkbox field="DeleteObsolete" caption="Remove obsolete tags/label groups from destination" hint="If a specific tag/label group is not present in the import file, will be removed from the destination system" value="true"/><br/><br/>
      </v:widget-block> 
    </v:widget>
    
    <v:alert-box type="warning" clazz="update-linked-entities-warning hidden" title="IMPORTANT">
      <i><b>IMPORTANT: Removing obsolete tags/label groups can result in modifying linked entities behavior.</b></i>
    </v:alert-box>
  </div>
...
$(document).ready(function() {
  var $dlg = $("#tag-snapp-import-dialog");
  ...
  $("#tag-snapp-import-dialog").find("[name='UpdateExistingTags']").prop('checked', false);
  $("#tag-snapp-import-dialog").find("[name='DeleteObsolete']").prop('checked', false);
  $("#tag-snapp-import-dialog").find("[name='DeleteObsolete']").change(refreshWarning);
});

function refreshWarning() {
  var $warnDiv = $("#tag-snapp-import-dialog").find(".update-linked-entities-warning");
  $warnDiv.setClass("hidden", !$("#tag-snapp-import-dialog").find("[name='DeleteObsolete']").isChecked());
}

function getImportParams() {
  return {
    Tag: {
    	UpdateExistingTags: $("#tag-snapp-import-dialog [name='UpdateExistingTags']").isChecked(),
    	DeleteObsolete: $("#tag-snapp-import-dialog [name='DeleteObsolete']").isChecked()
    }
  }
}

function doUploadFinish(obj) {
  resizeDialogWidth("#tag-snapp-import-dialog", 800);
  ...
  vgsService("AsyncProcess", reqDO, false, function(ansDO) {
	  var validImport = ansDO.Answer.SnAppImportPreview.ImportTypeCode == <%=LkSNExportType.Tag.getCode()%>;     
    ...
    if (validImport) {
      ...
      $('text[name=ImportTypeExpected]').append(<%=JvString.jsString(LkSNExportType.Tag.getHtmlDescription(pageBase.getLang()))%> + "<br/>"); 
...
function doImportTags() {
  var reqDO = {
      ...
      SnAppImport: {
        RepositoryId: repositoryId,
        AsyncProcessClassAlias: <%=JvString.jsString(AsyncProcessUtils.CLASS_ALIAS_IMPORT_SNP_TAG)%>, 
  ...
  vgsService("AsyncProcess", reqDO, false, function(ansDO) {
    ...
    $("#tag-snapp-import-dialog").dialog("close");
  });
}
