...
<v:dialog id="salechannel-snapp-import-dialog" title="Import sale channels" width="800" height="490" autofocus="false">
  <div id="step-upload">
    ...
    <v:alert-box type="info" title="@Common.Info" style="max-height:350px;overflow:auto">
      This wizard will import sale channels from a SnApp file into the system.<br/>
    </v:alert-box>
  </div>
  ...
  <div id="step-preview" class="v-hidden">
    ...
    <v:widget caption="@Common.Parameters">
      <v:widget-block>
        <v:db-checkbox field="UpdateExistingSaleChannels" caption="Update existing sale channels" value="true"/><br/><br/>
      </v:widget-block> 
    </v:widget>
  </div>
...
$(document).ready(function() {
  var $dlg = $("#salechannel-snapp-import-dialog");
  ...
  $("#salechannel-snapp-import-dialog").find("[name='UpdateExistingSaleChannels']").prop('checked', true);
...
function getImportParams() {
  return {
    SaleChannel: {
    	UpdateExistingSaleChannels: $("#salechannel-snapp-import-dialog [name='UpdateExistingSaleChannels']").isChecked()
    }
  }
}

function doUploadFinish(obj) {
  resizeDialogWidth("#salechannel-snapp-import-dialog", 800);
  ...
  vgsService("AsyncProcess", reqDO, false, function(ansDO) {
    var validImport = ansDO.Answer.SnAppImportPreview.ImportTypeCode == <%=LkSNExportType.SaleChannel.getCode()%>;     
    ...
    if (validImport) {
      ...
      $('text[name=ImportTypeExpected]').append(<%=JvString.jsString(LkSNExportType.SaleChannel.getHtmlDescription(pageBase.getLang()))%> + "<br/>"); 
...
function doImportSaleChannels() {
  var reqDO = {
      Command: "SnAppImport",
      SnAppImport: {
        RepositoryId: repositoryId,
        AsyncProcessClassAlias: <%=JvString.jsString(AsyncProcessUtils.CLASS_ALIAS_IMPORT_SNP_SALECHANNEL)%>, 
  ...
  vgsService("AsyncProcess", reqDO, false, function(ansDO) {
    ...
    $("#salechannel-snapp-import-dialog").dialog("close");
  });
}
