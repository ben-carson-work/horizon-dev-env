...
<v:dialog id="ratecode-snapp-import-dialog" title="Import rate codes" width="800" height="490" autofocus="false">
  <div id="step-upload">
    ...
    <v:alert-box type="info" title="@Common.Info" style="max-height:350px;overflow:auto">
      This wizard will import rate codes from a SnApp file into the system.<br/>
    </v:alert-box>
  </div>
  ...
  <div id="step-preview" class="v-hidden">
    ...
    <v:widget caption="@Common.Parameters">
      <v:widget-block>
        <v:db-checkbox field="UpdateExistingRateCodes" caption="Update existing rate codes" value="true"/><br/><br/>
      </v:widget-block> 
    </v:widget>
  </div>
...
$(document).ready(function() {
  var $dlg = $("#ratecode-snapp-import-dialog");
  ...
  $("#ratecode-snapp-import-dialog").find("[name='UpdateExistingRateCodes']").prop('checked', true);
...
function getImportParams() {
  return {
    RateCode: {
      UpdateExistingRateCodes: $("#ratecode-snapp-import-dialog [name='UpdateExistingRateCodes']").isChecked()
    }
  }
}

function doUploadFinish(obj) {
  resizeDialogWidth("#ratecode-snapp-import-dialog", 800);
  ...
  vgsService("AsyncProcess", reqDO, false, function(ansDO) {
    var validImport = ansDO.Answer.SnAppImportPreview.ImportTypeCode == <%=LkSNExportType.RateCode.getCode()%>;     
    ...
    if (validImport) {
      ...
      $('text[name=ImportTypeExpected]').append(<%=JvString.jsString(LkSNExportType.RateCode.getHtmlDescription(pageBase.getLang()))%> + "<br/>"); 
...
function doImportRateCodes() {
  var reqDO = {
      Command: "SnAppImport",
      SnAppImport: {
        RepositoryId: repositoryId,
        AsyncProcessClassAlias: <%=JvString.jsString(AsyncProcessUtils.CLASS_ALIAS_IMPORT_SNP_RATECODE)%>, 
  ...
  vgsService("AsyncProcess", reqDO, false, function(ansDO) {
    ...
    $("#ratecode-snapp-import-dialog").dialog("close");
  });
}
