...
<v:dialog id="gatecategory-snapp-import-dialog" title="Import gate categories" width="800" height="490" autofocus="false">
  <div id="step-upload">
    <v:widget caption="File upload">
      <v:widget-block>
        <jsp:include page="/resources/admin/repository/file_upload_widget.jsp"/> 
      </v:widget-block>
    </v:widget>
  
    <v:alert-box type="info" title="@Common.Info" style="max-height:350px;overflow:auto">
      This wizard will import gate categories from a SnApp file into the system.<br/>
    </v:alert-box>
  </div>
  
  <div id="step-preview" class="v-hidden">
    <jsp:include page="/resources/admin/snapp_import_geninfo_widget.jsp"/>

    <v:widget caption="@Common.Parameters">
      <v:widget-block>
        <v:db-checkbox field="UpdateExistingGateCategories" caption="Update existing gate categories" value="true"/><br/><br/>
      </v:widget-block> 
    </v:widget>
  </div>
  
  <jsp:include page="/resources/admin/snapp_import_invalidimport_widget.jsp"/>
...
$(document).ready(function() {
  var $dlg = $("#gatecategory-snapp-import-dialog");
  $dlg.on("snapp-dialog", function(event, params) {
	...
	$("#gatecategory-snapp-import-dialog").find("[name='UpdateExistingGateCategories']").prop('checked', true);
...
function getImportParams() {
  return {
	  GateCategory: {
    	UpdateExistingGateCategories: $("#gatecategory-snapp-import-dialog [name='UpdateExistingGateCategories']").isChecked()
    }
  }
}

function doUploadFinish(obj) {
  resizeDialogWidth("#gatecategory-snapp-import-dialog", 800);
  ...
  vgsService("AsyncProcess", reqDO, false, function(ansDO) {
	  var validImport = ansDO.Answer.SnAppImportPreview.ImportTypeCode == <%=LkSNExportType.GateCategory.getCode()%>;     
    ...
    if (validImport) {
      ...
      $('text[name=ImportTypeExpected]').append(<%=JvString.jsString(LkSNExportType.GateCategory.getHtmlDescription(pageBase.getLang()))%> + "<br/>"); 
...
function doImportGateCategories() {
  var reqDO = {
      Command: "SnAppImport",
      SnAppImport: {
        RepositoryId: repositoryId,
        AsyncProcessClassAlias: <%=JvString.jsString(AsyncProcessUtils.CLASS_ALIAS_IMPORT_SNP_GATE_CATEGORIES)%>, 
  ...
  vgsService("AsyncProcess", reqDO, false, function(ansDO) {
    ...
    $("#gatecategory-snapp-import-dialog").dialog("close");
  });
}
