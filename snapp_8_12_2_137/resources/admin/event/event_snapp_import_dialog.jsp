...
<v:dialog id="event-snapp-import-dialog" title="Import events" width="800" height="490" autofocus="false">
  <div id="step-upload">
    ...
    <v:alert-box type="info" title="@Common.Info" style="max-height:350px;overflow:auto">
      This wizard will import events from a SnApp file into the system, the following operations for linked entities are supported:<br/><br/>
      Create
      <ul>
        <li><b>Tags</b></li>
        <li><b>Meta fields</b></li>
        <li><b>Event properties (Event categories)</b></li>
        <li><b>Entity tabs</b></li>
      </ul>
    </v:alert-box>
  </div>
  ...
  <div id="step-preview" class="v-hidden">
    ...
    <v:widget caption="@Common.Parameters">
      <v:widget-block>
        <v:db-checkbox field="UpdateExistingEvent" caption="Update existing events" value="true"/><br/><br/>
    ...
    <v:alert-box type="warning" clazz="update-linked-entities-warning hidden" title="IMPORTANT">
      <i><b>IMPORTANT: Updating linked entities can result in modifying other events behavior.</b></i>
    </v:alert-box>
  </div>
...
$(document).ready(function() {
  var $dlg = $("#event-snapp-import-dialog");
  $dlg.on("snapp-dialog", function(event, params) {
	...
	$("#event-snapp-import-dialog").find("[name='UpdateExistingEvent']").prop('checked', true);
	$("#event-snapp-import-dialog").find("[name='UpdateLinkedEntities']").prop('checked', false);
	$("#event-snapp-import-dialog").find("[name='UpdateLinkedEntities']").change(refreshWarning);
});

function refreshWarning() {
	var $warnDiv = $("#event-snapp-import-dialog").find(".update-linked-entities-warning");
	$warnDiv.setClass("hidden", !$("#event-snapp-import-dialog").find("[name='UpdateLinkedEntities']").isChecked());
}

function getImportParams() {
  return {
    Event: {
    	UpdateExistingEvent: $("#event-snapp-import-dialog [name='UpdateExistingEvent']").isChecked(),
    	UpdateLinkedEntities: $("#event-snapp-import-dialog [name='UpdateLinkedEntities']").isChecked()
    }
  }
}

function doUploadFinish(obj) {
  resizeDialogWidth("#event-snapp-import-dialog", 800);
  ...
  vgsService("AsyncProcess", reqDO, false, function(ansDO) {
	  var validImport = ansDO.Answer.SnAppImportPreview.ImportTypeCode == <%=LkSNExportType.Event.getCode()%>;     
    ...
    if (validImport) {
      ...
      $('text[name=ImportTypeExpected]').append(<%=JvString.jsString(LkSNExportType.Event.getHtmlDescription(pageBase.getLang()))%> + "<br/>"); 
...
function doImportEvents() {
  var reqDO = {
      ...
      SnAppImport: {
        RepositoryId: repositoryId,
        AsyncProcessClassAlias: <%=JvString.jsString(AsyncProcessUtils.CLASS_ALIAS_IMPORT_SNP_EVENT)%>, 
  ...
  vgsService("AsyncProcess", reqDO, false, function(ansDO) {
    ...
    $("#event-snapp-import-dialog").dialog("close");
  });
}
