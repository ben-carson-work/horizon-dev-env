...
<v:dialog id="paymentprofile-snapp-import-dialog" title="Import payment profiles" width="800" height="490" autofocus="false">
  <div id="step-upload">
    ...
    <v:alert-box type="info" title="@Common.Info" style="max-height:350px;overflow:auto">
      This wizard will import payment profiles from a SnApp file into the system.<br/>
    </v:alert-box>
  </div>
  ...
  <div id="step-preview" class="v-hidden">
    ...
    <v:widget caption="@Common.Parameters">
      <v:widget-block>
        <v:db-checkbox field="UpdateExistingPaymentProfiles" caption="Update existing payment profiles" value="true"/><br/><br/>
      </v:widget-block> 
    </v:widget>
  </div>
...
$(document).ready(function() {
  var $dlg = $("#paymentprofile-snapp-import-dialog");
  ...
  $("#paymentprofile-snapp-import-dialog").find("[name='UpdateExistingPaymentProfiles']").prop('checked', true);
...
function getImportParams() {
  return {
    PaymentProfile: {
      UpdateExistingPaymentProfiles: $("#paymentprofile-snapp-import-dialog [name='UpdateExistingPaymentProfiles']").isChecked()
    }
  }
}

function doUploadFinish(obj) {
  resizeDialogWidth("#paymentprofile-snapp-import-dialog", 800);
  ...
  vgsService("AsyncProcess", reqDO, false, function(ansDO) {
    var validImport = ansDO.Answer.SnAppImportPreview.ImportTypeCode == <%=LkSNExportType.PaymentProfile.getCode()%>;     
    ...
    if (validImport) {
      ...
      $('text[name=ImportTypeExpected]').append(<%=JvString.jsString(LkSNExportType.PaymentProfile.getHtmlDescription(pageBase.getLang()))%> + "<br/>"); 
...
function doImportPaymentProfiles() {
  var reqDO = {
      Command: "SnAppImport",
      SnAppImport: {
        RepositoryId: repositoryId,
        AsyncProcessClassAlias: <%=JvString.jsString(AsyncProcessUtils.CLASS_ALIAS_IMPORT_SNP_PAYMENTPROFILE)%>, 
  ...
  vgsService("AsyncProcess", reqDO, false, function(ansDO) {
    ...
    $("#paymentprofile-snapp-import-dialog").dialog("close");
  });
}
