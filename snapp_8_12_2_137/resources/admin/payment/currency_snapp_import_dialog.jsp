...
<v:dialog id="currency-snapp-import-dialog" title="Import currencies" width="800" height="490" autofocus="false">
  <div id="step-upload">
    ...
    <v:alert-box type="info" title="@Common.Info" style="max-height:350px;overflow:auto">
      This wizard will import currencies from a SnApp file into the system.<br/>
    </v:alert-box>
  </div>
  ...
  <div id="step-preview" class="v-hidden">
    ...
    <v:widget caption="@Common.Parameters">
      <v:widget-block>
        <v:db-checkbox field="UpdateExistingCurrencies" caption="Update existing currencies" value="true"/><br/><br/>
      </v:widget-block> 
    </v:widget>
  </div>
...
$(document).ready(function() {
  var $dlg = $("#currency-snapp-import-dialog");
  ...
  $("#currency-snapp-import-dialog").find("[name='UpdateExistingCurrencies']").prop('checked', true);
...
function getImportParams() {
  return {
    Currency: {
    	UpdateExistingCurrencies: $("#currency-snapp-import-dialog [name='UpdateExistingCurrencies']").isChecked()
    }
  }
}

function doUploadFinish(obj) {
  resizeDialogWidth("#currency-snapp-import-dialog", 800);
  ...
  vgsService("AsyncProcess", reqDO, false, function(ansDO) {
    var validImport = ansDO.Answer.SnAppImportPreview.ImportTypeCode == <%=LkSNExportType.Currency.getCode()%>;     
    ...
    if (validImport) {
      ...
      $('text[name=ImportTypeExpected]').append(<%=JvString.jsString(LkSNExportType.Currency.getHtmlDescription(pageBase.getLang()))%> + "<br/>"); 
...
function doImportCurrencies() {
  var reqDO = {
      Command: "SnAppImport",
      SnAppImport: {
        RepositoryId: repositoryId,
        AsyncProcessClassAlias: <%=JvString.jsString(AsyncProcessUtils.CLASS_ALIAS_IMPORT_SNP_CURRENCY)%>, 
  ...
  vgsService("AsyncProcess", reqDO, false, function(ansDO) {
    showAsyncProcessDialog(ansDO.Answer.SnAppImport.AsyncProcessId, function() {
      triggerEntityChange(<%=LkSNEntityType.Currency.getCode()%>);    
    });
    $("#currency-snapp-import-dialog").dialog("close");
  });
}
