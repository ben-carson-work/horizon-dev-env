...
reqDO.Filters.AccountId.setString(pageBase.getSession().getOrgAccountId());
...
<% } %>
  ...
  <v:grid-row search="<%=ansDO%>" dateGroupFieldName="IssueDateTime" idFieldName="invoiceId">
    ...
    <td>
      ...
      <div class="list-subtitle"><%=invDO.InvoiceStatus.isLookup(LkSNInvoiceStatus.AutoSettled) ? "&mdash;" : invDO.InvoiceStatusDesc.getHtmlString()%>
      <% if (invDO.NegativePaymentCount.getInt() > 0) { %>
      &nbsp;<i class="fa fa-exclamation-triangle" style="color: <%=JvCLDef.Colors.clrOrange%>" title="<v:itl key="@Invoice.OrdersWithNegativePayments"/>"></i>
      <%} %>
      </div>
    </td>
    <td>
      ...
      <br/>
      <snp:datetime clazz="list-subtitle" timestamp="<%=invDO.IssueDateTime.getDateTime()%>" format="shortdatetime" timezone="local"/>
    </td> 
    <td>
      <div><%=pageBase.format(invDO.DueDate, pageBase.getShortDateFormat())%></div>
    </td>
    <td align="right">
